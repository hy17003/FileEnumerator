...
void CFileEnumeratorDlg::OnBnClickedBtnGenerate()
{
	...
	if (IsDirectoryExists(m_strPath))
	{
		...
		CString strFileName = m_strPath + "\\filelist.txt";
		char* filename = (LPSTR)(LPCSTR)strFileName;
		...
		file.close();
		strInfo.Format("生成文件列表成功,共找到%d个文件！", filelist.size());
	}
	else
	{
		strInfo = "无效目录！";
	}
	MessageBox(strInfo, "提示");
}
